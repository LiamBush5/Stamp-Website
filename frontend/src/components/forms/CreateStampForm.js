...
const CreateStampForm = ({ onSuccess, onCancel }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await stampsApi.createStamp(stampData);
            const createdStamp = response.data;

            if (formData.setId) {
                await setApi.addStampToSet(formData.setId, createdStamp.id);
            }
            ...
            setError('Failed to create stamp. Please try again.');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-75 overflow-y-auto h-full w-full flex items-center justify-center" style={{ zIndex: 1001 }}>
            <div className="bg-white rounded-lg shadow-2xl p-8 w-full max-w-4xl">
                <h3 className="text-3xl font-bold text-gray-900 mb-6">Add New Stamp</h3>
